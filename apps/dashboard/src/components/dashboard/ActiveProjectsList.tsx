...
function ProjectCard({ project }: { project: ProjectSummary }) {
  ...
  return (
    <Link
      ...
      className="group flex flex-col gap-4 rounded-2xl border border-border bg-card p-5 transition-colors hover:border-accent/40 hover:bg-card-hover"
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <h4 className="font-bold text-text">{project.name}</h4>
          <p className="mt-1 text-xs text-text-muted">tracked workspace</p>
        </div>
      ...
      <div className="space-y-1.5">
        ...
        <div className="h-2 w-full overflow-hidden rounded-full border border-border bg-bg-base">
