...
export default async function CreatePage() {
  ...
  const [roles, skills, profiles, sourceCatalog, marketplace, inventory, recommended] = await Promise.all([
  ...
  const sourceSummary = {
    ...
    recommended: recommended?.recommendations?.slice(0, 8) ?? [],
  };
  ...
  return (
    <>
      <TopBar title="Create" />
      <PageShell>
        <CreateRoleClient roles={roles} skills={skills} profiles={profiles.map((p) => p.profile)} sourceSummary={sourceSummary} />
      </PageShell>
    </>
  );
}
