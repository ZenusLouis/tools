...
const HeartbeatSchema = z.object({
  ...
  geminiAvailable: z.boolean().default(false),
  reportedModels: z.record(z.string(), z.array(z.string())).default({}),
...
export async function POST(req: NextRequest) {
  ...
  const device = await db.bridgeDevice.upsert({
    ...
    create: {
      ...
      geminiAvailable: parsed.data.geminiAvailable,
      reportedModels: parsed.data.reportedModels as Prisma.InputJsonValue,
      metadata,
      lastSeenAt: new Date(),
    },
    update: {
